...
def analyze_single_resume(resume_text: str, job_description: Optional[str], weighted_requirements: list[dict], jd_weight: float, technical_indicators: Optional[dict] = None, disqualifiers: Optional[list[str]] = None) -> dict:
    ...
    if active_disqualifiers:
        ...
        disqualifier_section += (
            ...
            "or \"Gurgaon\" or \"Noida\" or \"Kolkata\" or \"Ahmedabad\" IS a role based in India. \"London\" implies UK, \"Toronto\" implies Canada, etc.\n"
            "- For LOCATION conditions, the CANDIDATE'S CURRENT LOCATION IS NOT THE ONLY SIGNAL. Past roles matter too — check every role in the resume's "
            "work history against the condition, not just where the candidate lives now. A candidate currently in San Francisco who worked at TCS in Pune "
            "2019-2023 IS a match for \"worked in India in the past 6 years.\"\n"
            "- For an INDIA location disqualifier specifically, the following are all India-headquartered IT/outsourcing companies whose roles are presumed "
            "India-based unless the resume explicitly states a non-India office: Tata Consultancy Services / TCS, Infosys, Wipro, HCL Technologies / HCLTech, "
            "Tech Mahindra, Cognizant, LTIMindtree, L&T Infotech, Mphasis, Mindtree, Hexaware, Persistent Systems, Genpact, Zensar, Polaris, NIIT, MindTree, "
            "Larsen & Toubro Infotech. If a candidate's role at one of these companies has no city/office stated, treat it as India work.\n"
    ...
    if weighted_requirements:
        ...
        requirements_scoring += "  For Category A: the resume must show real EVIDENCE of the qualification, not just contain the keyword.\n\n"
        requirements_scoring += "  *** EVIDENCE vs CLAIM — APPLIES TO EVERY CATEGORY A REQUIREMENT ***\n"
        requirements_scoring += "  Candidates routinely use AI to inject the \"right\" keywords into their resumes — in Summary, Objective, Headline, "
        requirements_scoring += "Skills list, Tech Stack, Tools, About, and tagline bullets — for every job they apply to. Keyword presence is a CLAIM, "
        requirements_scoring += "NOT evidence of the qualification. Do not award credit on the basis of keyword presence anywhere on the resume.\n"
        requirements_scoring += "  Evidence for a Category A requirement lives in the Professional Experience / Work History section, where each role "
        requirements_scoring += "is anchored to a specific employer and a specific time range:\n"
        requirements_scoring += "    - Job title requirement -> the title must be the actual job title of a real role in the work history (not just in a tagline).\n"
        requirements_scoring += "    - Credential requirement -> the credential must appear in the Education or Certifications section, with school/issuer and date.\n"
        requirements_scoring += "    - Technology, methodology, or skill requirement -> the technology must appear in a work-history role's bullets, "
        requirements_scoring += "showing the candidate actually used it (built / shipped / deployed / owned), not just listed as a skill.\n"
        requirements_scoring += "    - Quantitative threshold (e.g. \"N+ years of X\") -> the qualifying roles in the work history must sum to at least N years.\n"
        requirements_scoring += "  The Summary, Objective, Headline, Skills, Tech Stack, Tools, and About sections are KEYWORD CLAIMS. They prove the "
        requirements_scoring += "candidate wants the term on their resume — not that they have the qualification. If the ONLY evidence for a Category A "
        requirements_scoring += "requirement is presence in one of those sections, the score is 0.\n"
        requirements_scoring += "  Apply this rule defensively. When in doubt, ask: \"Can I point to a specific role in the work history with an employer "
        requirements_scoring += "and dates that demonstrates this?\" If no, the score is 0.\n"
        requirements_scoring += "  *** END EVIDENCE vs CLAIM RULE ***\n\n"
        requirements_scoring += "  The next two sections are specific applications of the rule above:\n\n"
        ...
        requirements_scoring += "  *** CRITICAL ANTI-GAMING RULE FOR EXPERIENCE REQUIREMENTS ***\n"
        requirements_scoring += "  For requirements phrased as \"X experience\" or \"N+ years of X\" (e.g. \"2+ years of startup experience\", "
        requirements_scoring += "\"5+ years managing teams\", \"experience shipping consumer products\"), the evidence MUST come from the "
        requirements_scoring += "candidate's ACTUAL WORK HISTORY — a specific employer, a specific role, and a specific time period where the candidate did the thing.\n"
        requirements_scoring += "  Self-described traits, mindsets, summary buzzwords, or aspirational language are NOT experience and do NOT satisfy the requirement.\n"
        requirements_scoring += "  Examples of phrases that DO NOT count as evidence:\n"
        requirements_scoring += "    - For \"startup experience\": \"startup mentality\", \"entrepreneurial mindset\", \"scrappy and resourceful\", "
        requirements_scoring += "\"wears many hats\", \"thrives in fast-paced environments\", \"founder mindset\"\n"
        requirements_scoring += "    - For \"leadership experience\": \"leadership skills\", \"strong leader\", \"natural leader\"\n"
        requirements_scoring += "    - For \"consumer product experience\": \"passionate about consumer products\", \"user-obsessed\"\n"
        requirements_scoring += "    - For any \"X experience\": \"interested in X\", \"learning X\", \"familiar with X\"\n"
        requirements_scoring += "  For \"N+ years of X\" requirements, do this analysis:\n"
        requirements_scoring += "    1. Identify which roles in the work history actually demonstrate X (real employer + real role doing X).\n"
        requirements_scoring += "    2. Sum the durations of those qualifying roles.\n"
        requirements_scoring += "    3. Compare against the year threshold; if the sum is below the threshold, score 0.\n"
        requirements_scoring += "  Worked example: requirement \"2+ years of startup experience.\" Resume summary says \"startup mentality and fast-paced execution\" "
        requirements_scoring += "but every listed employer is a 1000+ person established company. Result: 0 points. Self-described mindset is NOT experience.\n"
        requirements_scoring += "  Worked example: same requirement, but resume work history shows two roles at ~20-person early-stage startups totaling 3 years. Result: FULL credit.\n"
        requirements_scoring += "  *** SPECIAL CASE: TECHNOLOGY KEYWORDS IN SKILLS SECTIONS ***\n"
        requirements_scoring += "  Bare technology keywords listed in a Skills, Tech Stack, Tools, or Summary section DO NOT count as experience for "
        requirements_scoring += "\"N+ years of X\" requirements. A candidate can list any technology they have casually touched; that is a CLAIM, not evidence of years of use.\n"
        requirements_scoring += "  For a \"N+ years of X\" requirement on a technology/methodology (RAG, Python, Kubernetes, LangChain, Snowflake, Terraform, etc.):\n"
        requirements_scoring += "    1. Look at the WORK HISTORY section (Professional Experience), not skills lists or summaries.\n"
        requirements_scoring += "    2. Identify which specific roles describe the candidate ACTUALLY USING the technology (built with it, deployed it, productionized it, owned it).\n"
        requirements_scoring += "    3. Sum those role durations.\n"
        requirements_scoring += "    4. If the sum is below the year threshold, score 0 — even if the technology is listed in the Skills section.\n"
        requirements_scoring += "  Worked example: requirement \"2+ years of RAG experience.\" Resume Skills section lists \"Python, RAG, LangChain, Pinecone\" "
        requirements_scoring += "and summary says \"experience with retrieval-augmented generation,\" but no role in the work history describes the candidate actually building "
        requirements_scoring += "or deploying a RAG system. Result: 0 points. A keyword in the skills list is a CLAIM, not 2 years of experience.\n"
        requirements_scoring += "  Worked example: same requirement, work history shows \"Built RAG-based search system at Acme (Jan 2023 - Jun 2024, 18 months)\" and "
        requirements_scoring += "\"Productionized a RAG chatbot at Beta Corp (Jul 2024 - present, 10 months).\" Total: ~28 months > 24. Result: FULL credit.\n"
        requirements_scoring += "  *** END EXPERIENCE ANTI-GAMING RULE ***\n\n"
...
- education_history: Array of {{"school": "<exact college/university name as it appears in the resume>", "degree": "<exact degree text as it appears, e.g. 'B.S. Computer Science' or 'Master of Business Administration'>"}} objects, one per school attended. Copy the strings VERBATIM from the resume so they can be located and highlighted in the original text.
